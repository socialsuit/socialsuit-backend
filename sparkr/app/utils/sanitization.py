"""Centralized input sanitization utilities for Sparkr.
...
SQL_INJECTION_PATTERN = re.compile(r'(\b(select|update|insert|delete|drop|alter|exec|union|create|where)\b)', re.IGNORECASE)
...
def sanitize_model(model: Union[BaseModel, SQLModel]) -> Union[BaseModel, SQLModel]:
    """Sanitize all string fields in a Pydantic or SQLModel model.
    ...
    Returns:
        The sanitized model
    """
    if not model or not (isinstance(model, BaseModel) or isinstance(model, SQLModel)):
        return model
...
def sanitize_sql_query(query: str) -> str:
    """Sanitize a SQL query string to prevent SQL injection.
    
    Note: This is a basic sanitization and should not be relied upon as the only
    defense against SQL injection. Always use parameterized queries.
    
    Args:
        query: The SQL query string to sanitize
        
    Returns:
        The sanitized SQL query string
    """
    if not query or not isinstance(query, str):
        return query
    
    # Flag potentially dangerous SQL keywords
    matches = SQL_INJECTION_PATTERN.findall(query)
    if matches:
        raise ValueError(f"Potentially unsafe SQL detected: {matches}")
